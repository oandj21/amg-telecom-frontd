    ...
    <div className="delete-confirm-overlay" onClick={onClose}>
      <div className="delete-confirm-container" onClick={e => e.stopPropagation()}>
        ...
        <div className="delete-confirm-footer">
          <button className="delete-confirm-btn delete-confirm-btn-cancel" onClick={onClose} disabled={deleting}>
            Annuler
          </button>
      ...
      <div className="pdf-modal-container" onClick={e => e.stopPropagation()}>
        <div className="pdf-modal-header">
          <div className="pdf-modal-title">
            ...
            <span>Aperçu du rapport - {pdfName?.length > 40 ? pdfName.slice(0, 37) + '...' : pdfName}</span>
          </div>
        ...
        <div className="pdf-modal-footer">
          ...
          <button onClick={openInNewTab} className="btn btn-secondary">
...
const EditReportModal = ({ isOpen, onClose, report, onSave, clients, user }) => {
  ...
  const addRow = () => {
    setRows([
      ...rows,
      { id: Date.now(), date: new Date().toISOString().slice(0, 10), clientName: '', quantity: 1 }
    ]);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      doc.text("RAPPORT D'ACTIVITÉ", pageWidth - margin, yPos + 4, { align: 'right' });
      
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.setTextColor(100, 116, 139);
      doc.text(`Date d'édition: ${new Date().toLocaleDateString('fr-FR')}`, pageWidth - margin, yPos + 10, { align: 'right' });
      ...
      doc.setFontSize(8);
      doc.setTextColor(30, 58, 138);
      doc.text("TECHNICIEN", rightColX, yPos);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.setTextColor(71, 85, 105);
      doc.text(`Nom: ${user?.name || 'Technicien'}`, rightColX, yPos + 5.5);
      doc.text(`ID: ${user?.id || '-'}`, rightColX, yPos + 10);
      doc.text(`Date: ${new Date().toLocaleDateString('fr-FR')}`, rightColX, yPos + 14.5);
      ...
      autoTable(doc, {
        startY: yPos,
        theme: 'plain',
        head: [['Date', 'Nom du client', 'Quantité']],
        body: tableRows,
      ...
      if (finalY + 30 < pageHeight) {
        ...
        doc.roundedRect(pageWidth - margin - 70, finalY, 70, 20, 5, 5, 'FD');
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(10);
        doc.setTextColor(30, 58, 138);
        doc.text("QUANTITÉ TOTALE", pageWidth - margin - 65, finalY + 7);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(14);
        doc.setTextColor(16, 185, 129);
        doc.text(`${totalQuantity}`, pageWidth - margin - 5, finalY + 16, { align: 'right' });
      } else {
        doc.addPage();
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
        doc.setTextColor(30, 58, 138);
        doc.text("RÉSUMÉ", margin, margin + 10);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(11);
        doc.setTextColor(16, 185, 129);
        doc.text(`Quantité totale: ${totalQuantity}`, margin, margin + 20);
      }
      ...
      doc.text(`${companyInfo.name} — Document généré par ${user?.name || 'Technicien'}`, margin, footerY);
      ...
      const response = await fetch(`${API_URL}/technician-reports/${report.id}`, {
        method: 'PUT',
        headers: {
      ...
      <div className="edit-modal-container" onClick={e => e.stopPropagation()}>
        ...
        <div className="edit-modal-body">
          ...
          <div style={{ 
            ...
            <button onClick={addRow} className="btn btn-outline" style={{ padding: '8px 16px', fontSize: '0.75rem' }}>
            ...
            <table className="report-table">
              <thead>
                <tr>
                  ...
                  <th className="col-client">Nom du client</th>
                  ...
                  <tr key={row.id}>
                    ...
                    <td>
                      <div style={{ position: 'relative' }}>
                        <input
                          type="text"
                          className="report-input client-name-input"
                        ...
                        {clientSearch && filteredClients.length > 0 && (
                          <div style={{
                            ...
                            {filteredClients.map(client => (
                              <div
                                key={client.id}
                                onClick={() => selectClientFromList(row.id, client.nom)}
                                style={{
                                  padding: '8px 12px',
                                  cursor: 'pointer',
                              ...
                              >
                                {client.nom}
                                {client.telephone && <span style={{ color: '#64748b', marginLeft: '8px' }}>{client.telephone}</span>}
                              </div>
                            ))}
                          </div>
                        )}
          ...
          <div className="summary-card" style={{ marginTop: '16px', marginBottom: 0 }}>
            <div>
              <span style={{ fontSize: '0.75rem', color: '#64748b' }}>Total lignes: </span>
              <strong>{rows.filter(r => r.clientName?.trim()).length}</strong>
            </div>
...
const TechnicianReport = () => {
  ...
  // Load saved reports from API (database)
  ...
  const removeRow = (id) => {
    if (rows.length > 1) {
      ...
      showToast('Gardez au moins une ligne', 'error');
    }
  };
  ...
  const generatePDF = async (saveToServer = true) => {
    const validRows = rows.filter(row => row.clientName && row.clientName.trim() && row.quantity > 0);
    if (validRows.length === 0) {
      showToast('Veuillez ajouter au moins une ligne valide (nom et quantité requis)', 'error');
      return;
    }
    ...
    try {
      ...
      if (saveToServer) {
        ...
        if (response.ok) {
          ...
          setRows([{ id: Date.now(), date: new Date().toISOString().slice(0, 10), clientName: '', quantity: 1 }]);
        } else {
  ...
  const viewSavedReport = (report) => {
    if (report.file_url) {
      setPreviewPdf({ url: report.file_url, name: report.file_name });
    } else {
      showToast('Impossible d\'afficher ce rapport', 'error');
    }
  };

  const openEditModal = (report, e) => {
    e.stopPropagation();
    setEditModal({ isOpen: true, report });
  };
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/technician-reports/${report.id}/download`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
  ...
  const openDeleteConfirm = (reportId, reportName, e) => {
    e.stopPropagation();
  ...
  const handleDeleteReport = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/technician-reports/${reportId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  // NEW: Automatically create specified number of rows
  const createMultipleRows = () => {
    const num = parseInt(numberOfLines);
    if (isNaN(num) || num <= 0) {
      showToast('Veuillez entrer un nombre valide (1 ou plus)', 'error');
      return;
    }
    ...
    setRows(newRows);
    showToast(`${num} ligne(s) créée(s) avec succès`, 'success');
  };
  ...
  return (
    <div className="tech-report-container">
      ...
      <DeleteConfirmModal
        ...
        message={`Êtes-vous sûr de vouloir supprimer le rapport "${deleteModal.reportName?.length > 50 ? deleteModal.reportName.slice(0, 47) + '...' : deleteModal.reportName}" ?`}
        deleting={deleting}
      />
      ...
      <div className="tech-report-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px', marginBottom: '16px' }}>
          <div>
            <h1 className="tech-report-title">📋 Rapports d'Activité</h1>
            <div className="tech-report-badge">
              <Smartphone size={14} /> Mode Technicien - {user?.name || 'Technicien'}
            </div>
          </div>
          <div className="header-actions">
            <button onClick={loadReportsFromServer} className="btn btn-outline" disabled={loadingReports}>
              <RefreshCw size={16} className={loadingReports ? 'spinner' : ''} />
              Actualiser
            </button>
        ...
        <div className="sync-status">
          <Database size={12} /> Rapports sauvegardés sur le serveur
        </div>
      </div>
      ...
      <div className="report-table-container">
        <div style={{ padding: '16px', borderBottom: '1px solid #eef2f6' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', marginBottom: '16px' }}>
            <div>
              <h3 style={{ fontSize: '0.9rem', fontWeight: 700, color: '#0f172a' }}>Nouveau rapport</h3>
              <p style={{ fontSize: '0.7rem', color: '#64748b' }}>Ajoutez les lignes d'activité avec nom client et quantité</p>
            </div>
            <button onClick={addRow} className="btn btn-outline">
              <Plus size={16} /> Ajouter 1 ligne
            </button>
          </div>
          
          {/* Bulk Row Creation Section */}
          <div style={{ 
            display: 'flex', 
            alignItems: 'center', 
            gap: '12px', 
            ...
            <div style={{ fontSize: '0.7rem', color: '#64748b', marginLeft: 'auto' }}>
              📋 {rows.length} ligne(s) actuelle(s)
            </div>
          </div>
        </div>
        ...
        <div className="summary-card" style={{ margin: 0, borderRadius: 0, borderTop: '1px solid #eef2f6' }}>
          <div>
            <span style={{ fontSize: '0.75rem', color: '#64748b' }}>Total lignes: </span>
            <strong style={{ color: '#1e293b' }}>{rows.filter(r => r.clientName?.trim()).length}</strong>
          </div>
          <div className="total-quantity">
            <span>Quantité totale:</span>
            <span>{calculateTotalQuantity()}</span>
          ...
          <button 
            ...
            className="btn btn-primary" 
            disabled={generating || rows.filter(r => r.clientName?.trim()).length === 0}
          >
            {generating ? <Loader size={16} className="spinner" /> : <Save size={16} />}
      ...
      <div style={{ marginTop: '32px' }}>
        <div className="section-title">
          ...
          <span className="tech-report-badge" style={{ background: '#f1f5f9', color: '#475569', marginLeft: 'auto' }}>
            {savedReports.length} rapport(s)
          </span>
        </div>
        ...
        {loadingReports ? (
          ...
          <div className="empty-state">
            ...
            <p style={{ fontSize: '0.7rem', marginTop: '8px' }}>Générez votre premier rapport ci-dessus</p>
          </div>
        ) : (
          <div className="saved-reports-grid">
            {savedReports.map((report) => (
              <div key={report.id} className="saved-report-card" onClick={() => viewSavedReport(report)}>
                <div className="saved-report-title">
                  ...
                  <div style={{ display: 'flex', gap: '4px' }}>
                    ...
                    <button 
                      ...
                      title="Télécharger"
                    >
                      {downloading === report.id ? <Loader size={14} className="spinner" /> : <Download size={14} />}
                    </button>
                    <button 
                      className="action-btn delete" 
                      onClick={(e) => openDeleteConfirm(report.id, report.file_name, e)}
                      title="Supprimer"
                    >
                      <Trash2 size={14} />
                    </button>
                ...
                <div style={{ fontSize: '0.7rem', color: '#334155' }}>
                  <div>👤 {user?.name || 'Technicien'}</div>
                  <div>📦 Quantité totale: <strong>{report.total_quantity}</strong></div>
                ...
                <div className="saved-report-meta">
                  ...
                  <span><Eye size={10} /> Cliquer pour voir</span>
                  <span><Edit size={10} /> Cliquer sur Modifier</span>
                </div>
              </div>
            ))}
          </div>
