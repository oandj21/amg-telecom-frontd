...
// ==================== ACTIVATIONS DETAILS MODAL (with 20‑item pagination) ====================
const ActivationsDetailsModal = ({ client, onClose, showToast }) => {
  ...
  // 🔹 NEW – Modal pagination state (20 per page)
  ...
  useEffect(() => {
    const loadClientActivations = async () => {
      ...
      try {
        ...
        // 🔹 CHANGED – added per_page=1000 to fetch ALL records
        const [activationsResponse, salesResponse] = await Promise.all([
          fetch(`${API_URL}/activations?client_id=${client.id}&per_page=1000`, {
            headers: { Authorization: `Bearer ${token}` }
          }),
          fetch(`${API_URL}/ventes?client_id=${client.id}&per_page=1000`, {
            headers: { Authorization: `Bearer ${token}` }
          })
        ]);
  ...
  // 🔹 NEW – Compute current page items (20 per page)
  ...
  // 🔹 NEW – Reset to page 1 when date filters change
  useEffect(() => {
    setModalPage(1);
  }, [startDate, endDate]);
            ...
            <>
              ...
              <div className="activations-table-container">
                <table className="activations-table">
                  ...
                  <tbody>
                    {/* 🔹 CHANGED – use currentModalItems instead of filteredItems */}
